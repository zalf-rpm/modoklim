...
DATA_GRID_SILICATE = "germany/silicate_allocation_dummy_25832_etrs89-utm32n.asc"
DATA_SOIL_CHARACTERISTICS = "params/soil/SoilCharacteristicData.json"
DATA_SOIL_CHARACTERISTIC_MODIFIERS = "params/soil/SoilCharacteristicModifier.json"
SILICATE_FC_PV_FACTOR = 1.1
...
def load_wessolek2009_tables(path_to_data, path_to_modifiers):
    def values(row):
        field_capacity = float(row["fieldCapacity"])
        return {
            "fc": field_capacity,
            "pv": field_capacity + float(row["airCapacity"]),
            "pwp": field_capacity - float(row["nFieldCapacity"])
        }

    with open(path_to_data) as file_:
        data = json.load(file_)["list"]
    with open(path_to_modifiers) as file_:
        modifiers = json.load(file_)["list"]

    data_by_texture_density = {
        (row["soilType"].upper(), int(float(row["soilRawDensity"]) / 100.0)): values(row)
        for row in data
    }
    modifiers_by_texture_organic_matter = {
        (row["soilType"].upper(), int(float(row["organicMatter"]) * 10.0)): values(row)
        for row in modifiers
    }
    return data_by_texture_density, modifiers_by_texture_organic_matter


def wessolek2009_fc_pv(layer, data, modifiers):
    """Calculate FC and pore volume exactly like MONICA's default Wessolek2009 PTF."""
    def value(parameter, default=None):
        parameter_value = layer.get(parameter)
        if parameter_value is None:
            return default
        return parameter_value[0] if isinstance(parameter_value, list) else parameter_value

    def principal_values(texture, raw_density):
        density_key = int(raw_density * 10.0)
        step = 2 if density_key < 15 else -2
        while (texture, density_key) not in data and 11 <= density_key <= 19:
            density_key += step
        try:
            return data[(texture, density_key)]
        except KeyError as error:
            raise ValueError(
                "No Wessolek2009 data for texture {} and raw density {}".format(
                    texture, raw_density)) from error

    def modifier_values(texture, organic_matter):
        key = (texture, int(organic_matter * 10.0))
        if key in modifiers:
            return modifiers[key]
        # MONICA treats textures absent from the modifier table as zero modification.
        if not any(modifier_texture == texture for modifier_texture, _ in modifiers):
            return {"fc": 0.0, "pv": 0.0, "pwp": 0.0}
        raise ValueError(
            "No Wessolek2009 modifier for texture {} and organic matter {}".format(
                texture, organic_matter))

    def interpolate(lower_value, upper_value, value_, lower_bound, upper_bound,
                    ignore_missing_bound=False):
        if ignore_missing_bound:
            if upper_value < 0.5 <= lower_value:
                return lower_value
            if lower_value < 0.5 <= upper_value:
                return upper_value
        if upper_bound != lower_bound:
            return ((value_ - lower_bound) / (upper_bound - lower_bound)
                    * (upper_value - lower_value) + lower_value)
        return lower_value

    texture = layer["KA5TextureClass"].upper()
    raw_density = float(value("SoilRawDensity")) / 1000.0
    stone_content = float(value("Sceleton", 0.0))
    if stone_content > 0.0:
        stone_content = min(stone_content, 0.8)
    organic_matter = value("SoilOrganicMatter")
    if organic_matter is None:
        # SoilOrganicCarbon uses mass% [0-100] in the producer profile.
        organic_matter_percent = float(value("SoilOrganicCarbon")) / soil_io3.SOM_to_C
    else:
        # SoilOrganicMatter uses mass% [0-1] in the producer profile.
        organic_matter_percent = float(organic_matter) * 100.0

    if raw_density < 1.1:
        density_lower = density_upper = 1.1
    elif raw_density < 1.3:
        density_lower, density_upper = 1.1, 1.3
    elif raw_density < 1.5:
        density_lower, density_upper = 1.3, 1.5
    elif raw_density < 1.7:
        density_lower, density_upper = 1.5, 1.7
    elif raw_density < 1.9:
        density_lower, density_upper = 1.7, 1.9
    else:
        density_lower = density_upper = 1.9

    if texture in ("HH", "HN"):
        density_lower = density_upper = -1.0

    if organic_matter_percent < 1.0:
        organic_lower = organic_upper = 0.0
    elif organic_matter_percent < 1.5:
        organic_lower, organic_upper = 0.0, 1.5
    elif organic_matter_percent < 3.0:
        organic_lower, organic_upper = 1.5, 3.0
    elif organic_matter_percent < 6.0:
        organic_lower, organic_upper = 3.0, 6.0
    elif organic_matter_percent < 11.5:
        organic_lower, organic_upper = 6.0, 11.5
    else:
        organic_lower = organic_upper = 11.5

    if texture in ("HH", "HN"):
        organic_lower = organic_upper = 0.0

    lower = principal_values(texture, density_lower)
    upper = principal_values(texture, density_upper)
    unmodified = {
        parameter: interpolate(lower[parameter], upper[parameter], raw_density,
                               density_lower, density_upper, ignore_missing_bound=True)
        for parameter in ("fc", "pv")
    }

    lower_modifier = ({"fc": 0.0, "pv": 0.0}
                      if organic_lower == 0.0
                      else modifier_values(texture, organic_lower))
    upper_modifier = ({"fc": 0.0, "pv": 0.0}
                      if organic_upper == 0.0
                      else modifier_values(texture, organic_upper))
    modified = {
        parameter: interpolate(lower_modifier[parameter], upper_modifier[parameter],
                               organic_matter_percent, organic_lower, organic_upper)
        for parameter in ("fc", "pv")
    }

    stone_factor = 1.0 - stone_content
    return {
        parameter: (unmodified[parameter] + modified[parameter]) / 100.0 * stone_factor
        for parameter in ("fc", "pv")
    }


def increase_fc_and_pore_volume(soil_profile, factor, wessolek_data, wessolek_modifiers):
    profile = deepcopy(soil_profile)
    for layer in profile:
        calculated = None
        for parameter, ptf_parameter in (("FieldCapacity", "fc"), ("PoreVolume", "pv")):
            current_value = layer.get(parameter)
            if current_value is None:
                if calculated is None:
                    calculated = wessolek2009_fc_pv(layer, wessolek_data, wessolek_modifiers)
                layer[parameter] = [calculated[ptf_parameter] * factor, "vol% [0-1]"]
            elif isinstance(current_value, list):
                current_value[0] *= factor
            else:
                layer[parameter] *= factor
    return profile
...
def run_producer(server = {"server": None, "port": None}, shared_id = None):
    ...
    wessolek_data, wessolek_modifiers = load_wessolek2009_tables(
        paths["path-to-data-dir"] + DATA_SOIL_CHARACTERISTICS,
        paths["path-to-data-dir"] + DATA_SOIL_CHARACTERISTIC_MODIFIERS)
    ...
    # silicate data
    path_to_silicate_grid = paths["path-to-data-dir"] + DATA_GRID_SILICATE
    silicate_metadata, _ = Mrunlib.read_header(path_to_silicate_grid)
    silicate_grid = np.loadtxt(path_to_silicate_grid, dtype=int, skiprows=6)
    print("read: ", path_to_silicate_grid)
    ...
    for _, setup_id in enumerate(run_setups):
        ...
        for srow in range(0, srows):
            ...
            for scol in range(0, scols):
                ...
                if silicate_grid[srow, scol] == 1:
                    if soil_id not in silicate_soil_id_cache:
                        silicate_soil_id_cache[soil_id] = increase_fc_and_pore_volume(
                            soil_profile, SILICATE_FC_PV_FACTOR,
                            wessolek_data, wessolek_modifiers)
                    soil_profile = silicate_soil_id_cache[soil_id]
