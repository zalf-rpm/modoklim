...
def run_producer(server = {"server": None, "port": None}, shared_id = None):
    ...
    # district data (ger.: Landkreise)
    path_to_district_grid = paths["path-to-data-dir"] + DATA_GRID_DISTRICTS
    district_epsg_code = int(path_to_district_grid.split("/")[-1].split("_")[2])
    district_crs = CRS.from_epsg(district_epsg_code)
    if district_crs not in soil_crs_to_x_transformers:
        soil_crs_to_x_transformers[district_crs] = Transformer.from_crs(soil_crs, district_crs)
    district_meta, _ = Mrunlib.read_header(path_to_district_grid)
    district_grid = np.loadtxt(path_to_district_grid, dtype=int, skiprows=6)
    district_interpolate = Mrunlib.create_ascii_grid_interpolator(district_grid, district_meta)
    print("read: ", path_to_district_grid)
    ...
    for _, setup_id in enumerate(run_setups):
        ...
        for srow in range(0, srows):
            ...
            for scol in range(0, scols):
                ...
                district_id = int(district_grid[srow, scol])
                ...
                if seed_harvest_cs:
                    ...
                    if seed_harvest_data:
                        ...
                        if setup["sowing-date"] == "fixed" and setup["harvest-date"] == "fixed":
                            ...
                            worksteps[-1]["date"] = "{:04d}-{:02d}-{:02d}".format(hds[0], calc_harvest_date.month,
