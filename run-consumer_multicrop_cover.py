...
PATHS = {
    "re-local-remote": {
        ...
        "path-to-output-dir": "D:/modoklim/asc_out/",
        "path-to-csv-output-dir": "D:/modoklim/csv_out/"
    },
    "mbm-local-remote": {
        "path-to-data-dir": "data/",
...
def write_row_to_grids(row_col_data, row, ncols, header, path_to_output_dir, path_to_csv_output_dir, setup_id):
    ...
    if row in row_col_data:
        for col in range(0, ncols):
                        ...
                        if key in data:
                            ...
                            if isinstance(v, list):
                                for i, v_ in enumerate(v):
                                    ...
                                    cmc_and_year_to_vals[year][f"{key}_{i + 1}"].append(v_)
                            else:
                                # cmc_and_year_to_vals[(cm_count, year)][key].append(v)
                                cmc_and_year_to_vals[year][key].append(v)
            ...
            for year, key_to_vals in cmc_and_year_to_vals.items():
    ...
    for key, y2d_ in output_grids.items():
        ...
        for year, row_arr in y2d.items():
            key2 = key.replace("/", "_")
            path_to_file = f"{path_to_output_dir}{key2}_{year}.asc"
...
def run_consumer(leave_after_finished_run=True, server={"server": None, "port": None}, shared_id=None):
    ...
    expected_setup_count = None
    finished_setup_ids = set()

    def process_message(msg):
        nonlocal expected_setup_count

        custom_id = msg.get("customId", {})
        reported_count = custom_id.get("expected_setup_count")

        if reported_count is not None:
            reported_count = int(reported_count)

            if expected_setup_count is None:
                expected_setup_count = reported_count
            elif expected_setup_count != reported_count:
                raise ValueError("Conflicting expected setup counts")
        ...
        if not write_normal_output_files:
            ...
            while (data["next-row"] in data["row-col-data"] and data["datacell-count"][data["next-row"]] == 0) \
                ...
                setup_finished = (
                    (data["end_row"] < 0 and data["next-row"] > data["nrows"] - 1)
                    or (0 <= data["end_row"] < data["next-row"]))

                if setup_finished:
                    if setup_id not in finished_setup_ids:
                        finalize_outputs(setup_id, data["nrows"], data["ncols"])
                        finished_setup_ids.add(setup_id)
                        print(
                            f"Setup {setup_id} finished. Total setups finished: "
                            f"{len(finished_setup_ids)}/{expected_setup_count}"
                        )

                    return (
                        leave_after_finished_run
                        and expected_setup_count is not None
                        and len(finished_setup_ids) >= expected_setup_count
                    )
