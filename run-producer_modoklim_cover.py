...
def _parse_relative_date(value):
    """Map a MONICA relative date (0000/0001-MM-DD) to a Python date."""
...
def intermediate_crop_dates(latest_harvest_date, next_sowing_date):
    """Return relative sowing/removal dates, or None when the window is too short."""
    previous_harvest = _parse_relative_date(latest_harvest_date)
    next_sowing = _parse_relative_date(next_sowing_date)
    ...
    return ("{:04d}-{:02d}-{:02d}".format(intermediate_sowing.year - 2000,
                                          intermediate_sowing.month, intermediate_sowing.day),
            "{:04d}-{:02d}-{:02d}".format(intermediate_removal.year - 2000, intermediate_removal.month,
                                          intermediate_removal.day))
...
def make_intermediate_crop(template, latest_harvest_date, next_sowing_date):
    ...
    intermediate_crop = deepcopy(template)
    harvest_ws = next(ws for ws in intermediate_crop["worksteps"] if ws["type"][-7:] == "Harvest")
...
def run_producer(server = {"server": None, "port": None}, shared_id = None):
    ...
    for _, setup_id in enumerate(run_setups):
        ...
        intermediate_crop_json = deepcopy(crop_json)
        ...
        main_crop_template = env_template["cropRotation"][0]
        ...
        for srow in range(0, srows):
            ...
            for scol in range(0, scols):
                ...
                if "date" in sowing_ws and "latest-date" in harvest_ws:
                    intermediate_crop = make_intermediate_crop(intermediate_crop_template, harvest_ws["latest-date"],
                                                               sowing_ws["date"])
                    if intermediate_crop is not None:
                        main_crop["worksteps"].extend(intermediate_crop["worksteps"])
